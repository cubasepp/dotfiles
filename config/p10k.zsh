# Generated by Powerlevel10k configuration wizard on 2026-08-21 at 13:17 CEST.
...
() {
  ...
  # Context color in SSH without privileges. Bright orange -- deliberately loud so a
  # remote shell is unmistakable at a glance, distinct from root's red and the plain
  # grey it would otherwise share with the (normally hidden) local default.
  ...
  # Context format when in SSH without privileges: bold user@hostname, so it stands
  # out the same way root does.
